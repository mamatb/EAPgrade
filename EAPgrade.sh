...
then
    echo '[!] internet access and name resolution needed in order to clone EAPHammer' >&2
    exit 1
else
...
then
    echo '[!] git installation needed in order to use EAPgrade.sh' >&2
    exit 1
else
...
echo '[+] setting up user "EAPgrade" to access the fake AP network with password "changeme"' >&2
...
echo '[+] done! Now the WPA/WPA2-MGT fake AP attack should launch automatically after booting, raw logs will be located at "'"${EAPHAMMER_DIR}"'/logs/hostapd-eaphammer.raw". Next steps:' >&2
